...
function App(){
  ...
  const [scenes,setScenes]=useState([]),[activeScene,setActiveScene]=useState(''),[editScene,setEditScene]=useState(''),[selected,setSelected]=useState(null),[adding,setAdding]=useState(false),[syncState,setSyncState]=useState({applying:false,appliedKey:'',failedKey:'',error:'',retrying:false}),[previewError,setPreviewError]=useState('');
  ...
  useEffect(()=>{
    ...
    setScenes(layout.scenes.map(scene=>({...scene,layers:scene.layers.map(withUid)})));setActiveScene(layout.activeScene);setEditScene(layout.activeScene);setMic(layout.microphoneId||'');setDesktop(layout.desktopId||'');setPortrait(!!layout.portrait);setTransition(TRANSITION_LABELS[layout.transition?.style]?{style:layout.transition.style,durationMs:layout.transition.durationMs}:DEFAULT_TRANSITION);
    if(layout.microphoneId)initialDevices.current.microphone=true;
  },[state.user?.id,state.layout]);
  // The scene on air is what the engine composes; the scene being edited is what the Fontes dock shows.
  // They are the same until another scene is chosen in the dock, so a scene can be prepared while one is live.
  const scene=scenes.find(item=>item.id===activeScene)||scenes[0];const programLayers=scene?.layers||[];
  const edited=scenes.find(item=>item.id===editScene)||scene;const layers=edited?.layers||[];const editingProgram=edited?.id===scene?.id;
  editScope.current={userId:state.user?.id,sceneId:edited?.id,selected};
  const patchScene=(id,change)=>setScenes(current=>current.map(item=>item.id===id?{...item,...change(item)}:item));
  const setLayers=updater=>{if(edited)patchScene(edited.id,item=>({layers:updater(item.layers)}));};
  ...
  useEffect(()=>{
    ...
    suggestedScenes.current.add(scene.id);initialDevices.current.camera=true;patchScene(scene.id,()=>({layers:[withUid({kind:'camera',id:String(firstCamera.id),fit:'fit',corner:'br',size:.3,visible:true,name:''})]}));
  },[devices,scene?.id]);
  ...
  const missingLayers=programLayers.filter(layerMissing);
 ...
 // the two scenes hold the same sources, and that is what the engine animates.
  const composition=useMemo(()=>JSON.stringify({scene:scene?.id||'',layers:programLayers.map(toEngine),mic,desktop,portrait,transition}),[scene?.id,programLayers,mic,desktop,portrait,transition]);
  const canApply=!!state.user&&accountScope.current===state.user.id&&layoutLoaded.current&&!!scene&&!missingLayers.length&&(!programLayers.length||!missingMic&&!missingDesktop);
  useLayoutEffect(()=>{
    synchronizer.update({scope:state.user?.id,key:composition,payload:{sceneId:scene?.id,transition,layers:programLayers.map(toEngine),microphoneId:mic,desktopId:desktop,portrait},valid:canApply,enabled:!!state.prepared,blocked:!!(localBusy||state.busy)});
  ...
  const removeLayer=layer=>{
    suggestedScenes.current.add(edited?.id);
    if(editingProgram&&layers.length===1&&(missingMic||missingDesktop)&&syncState.appliedPayload){setMic(syncState.appliedPayload.microphoneId);setDesktop(syncState.appliedPayload.desktopId);setSourceNotice('Removendo a última fonte. A seleção de áudio anterior foi mantida para aplicar o quadro preto; confira os medidores.');}
  ...
  // It opens for editing without going on air; the scene list puts it on air when the streamer clicks it.
  const openScene=()=>{setSelected(null);setAdding(false);setSceneEdit(true);};
  const editOnly=id=>{setEditScene(id);setSelected(null);setAdding(false);};
  const addScene=(copyFrom=null)=>{const id='cena-'+uid();suggestedScenes.current.add(id);setSceneEdit(false);setScenes(list=>[...list,{id,name:copyFrom?'Cópia de '+copyFrom.name:'Cena '+(list.length+1),layers:(copyFrom?.layers||[]).map(layer=>withUid(persistLayer(layer)))}]);if(!scene)setActiveScene(id);editOnly(id);};
  const removeScene=item=>{if(scenes.length<2)return;const fallback=scenes.find(entry=>entry.id!==item.id).id;setScenes(list=>list.filter(entry=>entry.id!==item.id));if(scene?.id===item.id)setActiveScene(fallback);if(edited?.id===item.id)editOnly(scene?.id===item.id?fallback:scene.id);setSelected(null);setAdding(false);setSceneEdit(false);};
  ...
  const previewMessage=!state.prepared?(programLayers.length?'Clique em Abrir prévia para ligar a captura. A prévia aparece aqui antes de você entrar ao vivo.':'Adicione fontes ou abra a prévia com um quadro preto. O áudio usa os dispositivos selecionados.'):'';
  return <main className="studio"><header className="topbar"><div className="brand"><span className="brand-mark">p<span>×</span></span> Privex <b>Studio</b><small>BETA</small></div><span className={`signal ${displayStatus==='live'?'is-live':''}`}><i/>{statusNames[displayStatus]||'Fora do ar'}</span><div className="account"><button className="text-button" disabled={busy||sessionActive||state.transmitting} onClick={()=>run('updates.check')}>Atualizações</button><span>@{state.user.username}</span><button title="Sair da conta" aria-label="Sair da conta" disabled={busy||active} onClick={()=>run('logout')}><LogOut size={18}/></button></div></header>
    ...
    <div className="workspace"><section className="production">
      <div className="preview-shell"><div className="preview-stage"><div ref={preview} className={displayPortrait?'preview is-portrait':'preview'}>{!state.prepared&&<div className="preview-empty"><Video size={34}/><h2>{programLayers.length?'Prévia desligada':'Monte sua cena'}</h2><p>{previewMessage}</p></div>}</div></div>
      ...
      <div className="docks">
        <section className="dock dock-scenes" aria-label="Cenas"><header><h3><Film size={14}/> Cenas</h3><button className="icon-button" aria-label="Nova cena" title="Nova cena, sem fontes" disabled={scenes.length>=12} onClick={()=>addScene()}><Plus size={15}/></button></header>
          <ul className="scene-list">{scenes.map(item=><li key={item.id} className={(item.id===scene?.id?'is-active':'')+(item.id===edited?.id&&!editingProgram?' is-editing':'')}><button className="scene-button" title={item.id===scene?.id?'No ar':'Clique para colocar no ar'} onClick={()=>{setActiveScene(item.id);editOnly(item.id);}} onDoubleClick={()=>{setActiveScene(item.id);editOnly(item.id);openScene();}}>{item.name}{item.id===edited?.id&&!editingProgram&&<small className="scene-tag">editando</small>}</button>{item.id===edited?.id&&<button className="icon-button" aria-label="Editar cena" title="Nome, troca de cena, duplicar e remover" onClick={openScene}><Pencil size={13}/></button>}</li>)}</ul>
          <p className="dock-hint">Clique para trocar, inclusive ao vivo. Para mexer nas fontes de outra cena sem trocar, escolha a cena na lista de Fontes.</p>{layoutSave.error?<div className="dock-hint" role="alert">Não foi possível salvar as cenas: {layoutSave.error}<button className="text-button" onClick={layoutSave.retry}>Tentar salvar novamente</button></div>:layoutSave.pending&&<p role="status" className="dock-hint">Salvando alterações…</p>}
        </section>
        <section className="dock dock-sources" aria-label="Fontes"><header><h3><Layers size={14}/> Fontes</h3><label className="scene-pick" title="A cena destas fontes. Escolher outra cena aqui não troca a cena no ar."><span className="sr-only">Cena em edição</span><select aria-label="Cena em edição" value={edited?.id||''} disabled={!scenes.length} onChange={e=>editOnly(e.target.value)}>{scenes.map(item=><option key={item.id} value={item.id}>{item.name}{item.id===scene?.id?' · no ar':''}</option>)}</select></label><div className="dock-actions"><button className="icon-button" aria-label={devices?'Atualizar equipamentos':'Buscar equipamentos'} title={devices?'Atualizar lista de equipamentos':'Buscar equipamentos'} disabled={busy} onClick={enumerate}><RefreshCw size={14}/></button><button className="icon-button" aria-label="Adicionar fonte" aria-expanded={adding} disabled={busy||layers.length>=6} onClick={()=>setAdding(value=>!value)}><Plus size={15}/></button></div></header>
          {!editingProgram&&edited&&<p className="dock-hint is-editing" role="status">Editando <b>{edited.name}</b> fora do ar; a live continua em {scene?.name}. <button className="text-button" onClick={()=>setActiveScene(edited.id)}>Colocar no ar</button></p>}
          ...
          {layers.length?<ul className="layer-list">{layers.map((layer,index)=>{const Icon=KIND_ICONS[layer.kind],missing=layerMissing(layer);return <li key={layer.uid} className={(layer.uid===selected?'is-selected':'')+(layer.visible===false?' is-hidden':'')}>
            <button className="layer-main" onClick={()=>setSelected(layer.uid===selected?null:layer.uid)}><Icon size={14}/><span className="layer-name">{layerLabel(layer)}</span>{missing?<span className="layer-flag is-missing">desconectado</span>:state.prepared&&inSync&&editingProgram&&layer.visible!==false&&layerHolding(index)?<span className="layer-flag" title="A captura parou de enviar imagem; o último quadro continua no ar.">quadro congelado</span>:state.prepared&&inSync&&editingProgram&&layer.visible!==false&&layerReady(index)===false?<span className="layer-flag" title={layer.kind==='game'?'O jogo ainda não desenhou. Em "Qualquer jogo em tela cheia" ele precisa estar em primeiro plano e cobrindo o monitor inteiro; um jogo em 4:3 numa janela menor que a tela não é detectado. Escolha a janela do jogo na lista.':layer.kind==='window'?'A janela ainda não enviou imagem. Se for um jogo, ele precisa estar aberto e não minimizado. A cena já está aplicada; a imagem entra quando a janela desenhar.':'Sem imagem deste equipamento.'}>{layer.kind==='game'?'aguardando o jogo':layer.kind==='window'?'aguardando a janela':'sem sinal'}</span>:null}</button>
      ...
      {sceneEdit&&edited&&<><div className="sheet-backdrop" onClick={()=>setSceneEdit(false)}/><div className="layer-dialog" role="dialog" aria-modal="true" aria-label="Ajustes da cena" onKeyDown={e=>{if(e.key==='Escape')setSceneEdit(false);}}>
        <header><h3><Film size={15}/> {edited.name}</h3><span className="dock-hint">{editingProgram?'as mudanças valem na hora':'esta cena não está no ar'}</span><button className="icon-button" aria-label="Fechar ajustes da cena" onClick={()=>setSceneEdit(false)}><X size={16}/></button></header>
        <div className="layer-props">
          <label>Nome<input autoFocus aria-label="Nome da cena" maxLength={40} value={edited.name} onChange={e=>patchScene(edited.id,()=>({name:e.target.value}))} onBlur={e=>{if(!e.target.value.trim())patchScene(edited.id,item=>({name:'Cena '+(scenes.indexOf(item)+1)}));}}/></label>
        ...
        <footer><button className="secondary" disabled={scenes.length>=12} onClick={()=>addScene(edited)}><Copy size={14}/> Duplicar</button><button className="secondary" disabled={scenes.length<2} onClick={()=>{removeScene(edited);setSceneEdit(false);}}><Trash2 size={14}/> Remover cena</button><button className="primary" onClick={()=>setSceneEdit(false)}><Check size={15}/> Concluir</button></footer>
      </div></>}
      {selectedLayer&&(()=>{const Icon=KIND_ICONS[selectedLayer.kind];return <><div className="sheet-backdrop" onClick={()=>setSelected(null)}/><div className="layer-dialog" role="dialog" aria-modal="true" aria-label="Ajustes da fonte" onKeyDown={e=>{if(e.key==='Escape')setSelected(null);}}>
        ...
        <div className="layer-props">
            ...
            {selectedLayer.kind==='game'&&<p className="fine">Fonte antiga de captura por dentro do jogo, mantida para as cenas que já a usavam. Com "Qualquer jogo em tela cheia" o jogo precisa estar em primeiro plano e cobrir o monitor inteiro: um jogo em 4:3 numa janela menor que a tela não é detectado, e a fonte fica em "aguardando o jogo". Nesse caso escolha a janela do jogo nesta lista, ou use Janela / Jogo.</p>}
