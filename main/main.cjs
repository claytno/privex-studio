...
async function command(name,data){
  ...
  if(name==='bounds'){if(!prepared)return;const r=data||{};const host=win.webContents.getZoomFactor()*require('electron').screen.getDisplayMatching(win.getBounds()).scaleFactor;
    ...
    const scale=Number.isFinite(r.ratio)&&r.ratio>=0.5&&r.ratio<=8?r.ratio:host;const [cw,ch]=win.getContentSize();for(const k of ['x','y','width','height'])if(!Number.isFinite(r[k])||r[k]<0||r[k]>10000)throw new Error('Área inválida.');if(r.x+r.width>cw+5||r.y+r.height>ch+5)throw new Error('Área fora da janela.');const revision=++previewRevision;await engine.request('preview',{visible:false});if(revision!==previewRevision||!r.width||!r.height)return;await engine.request('resize',{bounds:Object.fromEntries(['x','y','width','height'].map(k=>[k,Math.round(r[k]*scale)]))});if(revision===previewRevision&&prepared)await engine.request('preview',{visible:true});return;}
