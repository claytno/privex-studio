...
async function command(name,data){
  ...
  if(['start','resume','mute','volume','scene','overlay','layer','layout.save','image.pick','preview.close','title'].includes(name)&&!user)throw new Error('Confirme a conta antes de usar o Studio.');
  ...
  if(name==='bounds'){if(!prepared)return;const r=data||{};const host=win.webContents.getZoomFactor()*require('electron').screen.getDisplayMatching(win.getBounds()).scaleFactor;
    // The page reports the ratio its own layout used; it is the one that matches these coordinates.
    const scale=Number.isFinite(r.ratio)&&r.ratio>=0.5&&r.ratio<=8?r.ratio:host;const [cw,ch]=win.getContentSize();for(const k of ['x','y','width','height'])if(!Number.isFinite(r[k])||r[k]<0||r[k]>10000)throw new Error('Área inválida.');if(r.x+r.width>cw+5||r.y+r.height>ch+5)throw new Error('Área fora da janela.');const revision=++previewRevision;await engine.request('preview',{visible:false});if(revision!==previewRevision||!r.width||!r.height)return;// The content size (device-independent pixels) lets the engine find where the page starts inside the window: Electron draws the menu bar above it, in the same client area.
    await engine.request('resize',{bounds:Object.fromEntries(['x','y','width','height'].map(k=>[k,Math.round(r[k]*scale)])),content:{width:cw,height:ch}});if(revision===previewRevision&&prepared)await engine.request('preview',{visible:true});return;}
  ...
  if(busy||heartbeatBusy&&['prepare','start','resume','logout'].includes(name))throw Object.assign(new Error('Aguarde a operação atual.'),{status:423});busy=true;failure='';emit();try{
    switch(name){
      ...
      case 'title':{if(!active())throw new Error('Abra uma live para alterar o título.');if(typeof data?.title!=='string')throw new Error('Título inválido.');const title=data.title.replace(/\s+/g,' ').trim();if(!title||title.length>100)throw new Error('Informe um título de 1 a 100 caracteres.');const result=await api('PUT',`/obs/v1/live/${studio.session.id}/title`,{title});if(studio?.session&&result?.session?.title)studio.session.title=result.session.title;notice='Título atualizado. Quem está assistindo vê o novo título.';return result;}
