...
| `prepare` | `layers` (1–8, index 0 in front) or the earlier single-source form `sourceType` camera/display/window with `cameraId`/`sourceId`; explicit `microphoneId`/`desktopId` or empty for silence; `muted`; width/height 1280×720,720×1280,1920×1080,1080×1920; fps30. Starts local capture only after user chooses. |
| layer object | `kind` camera/window/display/game (`id` must be in the current enumeration; for game, `any_fullscreen` or a window id, using the upstream graphics hook injected into the game process with `anti_cheat_hook`), image (`file`: absolute PNG/JPG/GIF/BMP/WebP up to 25 MB, existence checked) or text (`text` up to 200 characters, native GDI+ with outline); `fit` fit (whole image), fill (crop) or corner; `corner` tl/tr/bl/br; `size` 0.1–0.7 of the canvas width for corner boxes (16:9); `visible`. |
| `reconfigure` | Same fields while prepared, including during publishing. New capture sources must produce dimensions within 3 s; unchanged sources are reused (a camera is not reopened when another layer changes). On any failure the previous composition, audio and output stay untouched. |
| `layer` | `index`, `visible`: shows or hides one composed layer without recomposing. Pause still hides every layer. |
...
| `status` | state idle/ready/connecting/streaming/reconnecting; prepared, streaming, muted, canvas dimensions, primary source dimensions, `layers` (kind, visible, ready, width, height), totalBytes, droppedFrames, numeric stopCode. Never includes keys or raw network errors. |
| `shutdown` | Ends output/capture, frees media engine and exits. |

Full-canvas layers fit the whole source unless `fill` is requested explicitly. Preview fits the same canvas. Exact window title matching prevents fallback to another title or full desktop. A specific microphone is required rather than OS default to prevent changing to a different mic silently. Reconnection attempts are bounded to 5 retries ×3 seconds; backend remains authoritative and host must stop on revocation/session end. OBS presence is not proof of server playback.
...
- Load fixed module names from installation path only. Bundled media modules: win-dshow, win-wasapi, win-capture, obs-x264, obs-ffmpeg, obs-outputs, rtmp-services, image-source, obs-text. The win-capture data directory also ships the upstream graphics-hook DLLs, inject-helper and get-graphics-offsets executables required by game capture; window capture requests Windows Graphics Capture (method 2) and falls back to BitBlt where unsupported.
...
Physical device variations, prolonged RTMPS delivery, device removal during output, and end-to-end audio synchronization still require hardware validation. Native parenting is tested with the real Electron/engine bridge in an isolated fixture; final application modal/route positioning remains a UI integration concern. The engine composes up to eight layers (capture, image, text) with corner or full-canvas placement plus interval and native goal overlay; the self-test verifies corner composition, hiding and source reuse on the GPU. Free-form dragging, browser overlays, filters and transitions are not implemented by this binary.
