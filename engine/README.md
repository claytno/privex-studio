...
| layer object | `kind` camera/window/display (`id` must be in the current enumeration), image (`file`: absolute PNG/JPG/GIF/BMP/WebP up to 25 MB, existence checked) or text (`text` up to 200 characters, native GDI+ with outline); `fit` fit (whole image), fill (crop) or corner; `corner` tl/tr/bl/br; `size` 0.1–0.7 of the canvas width for corner boxes (16:9); `visible`. |
