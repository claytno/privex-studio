...
app.whenReady().then(async()=>{let win;try{
 ...
 assert.equal(await js("!!document.querySelector('[aria-label=\"Posição da fonte\"]')"),true,'The new source is selected for editing');
 ...
 await click('[aria-label="Adicionar fonte"]');await delay(50);await clickText('.add-panel button','Imagem');await delay(600);
