...
export default function LiveCommerceStudio({ sessionId }) {
  const { data, setData, error, loading, refresh } = useLivePolling(`/lives/${sessionId}/commerce`, 15000);
  ...
  const save = async event => {
    ...
    try {
      const response = await api.put(`/lives/${sessionId}/commerce`, { catalog_version: value.catalog_version, items: value.items.map(item => ({ id: item.id, kind: item.kind, title: item.title.trim(), amount_cents: parseLiveCents(item.price), delivery_seconds: Number(item.delivery_seconds) || 300, active: !!item.active, outcomes: item.kind === 'wheel' ? item.outcomesText.split('\n').map(label => ({ label: label.trim() })).filter(o => o.label) : [] })), goal: value.goal.title.trim() ? { title: value.goal.title.trim(), target_cents: parseLiveCents(value.goal.price), active: !!value.goal.active } : null }, { timeout: 20000 });
      setData(response.data); setDraft(null); setNotice('Interações atualizadas. Pedidos anteriores mantêm as condições originais.'); refresh();
  ...
  return <section className="min-w-0 space-y-5 [overflow-wrap:anywhere] rounded-2xl border border-px-border bg-px-card p-4 sm:p-6">
    <header><h2 className="text-lg font-semibold">Interações e ganhos</h2><p className="mt-2 text-sm text-px-muted">Crie presentes, ações e roleta. Todos os valores entram diretamente na sua carteira; a comissão continua apenas no saque.</p></header>
    ...
    <div className="border-t border-px-border pt-5"><LiveOrdersPanel sessionId={sessionId} creator /></div>
  </section>;
}
