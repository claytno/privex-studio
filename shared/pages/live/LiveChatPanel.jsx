...
export default function LiveChatPanel({ sessionId, overlay = false, condensed = false }) {
  ...
  return <section aria-label="Chat da live" className={`flex h-full min-h-0 min-w-0 flex-col overflow-hidden rounded-2xl ${overlay ? 'bg-black/65 text-white backdrop-blur-sm' : 'border border-px-border bg-px-card'} ${condensed ? 'p-2' : 'p-3'}`}>
    ...
    <form onSubmit={send} className="mt-2 flex shrink-0 items-end gap-2"><label className="min-w-0 flex-1"><span className="sr-only">Escrever no chat da live</span><textarea value={body} onChange={e => setBody(e.target.value)} maxLength={500} rows={!condensed && body.length > 70 ? 3 : 1} disabled={busy || !data?.can_send} placeholder={data?.can_send ? 'Escreva uma mensagem…' : 'Chat indisponível para envio'} className="max-h-28 min-h-11 w-full resize-none rounded-xl border border-white/20 bg-black/30 px-3 py-2 text-base text-white placeholder:text-white/65 disabled:opacity-60" /></label><Button type="submit" aria-label="Enviar mensagem" loading={busy} disabled={!data?.can_send || !body.trim()} className="min-h-11 min-w-11 px-2"><Send size={18} /></Button></form>
