...
import { Flag, MoreVertical, Send, Shield, Pin, Reply, Radio, Users, LogIn, BookOpen } from 'lucide-react';
...
import LiveAudiencePanel from "./LiveAudiencePanel.jsx";
import { liveChatTimeline } from "./liveAudienceUtils.js";
import { Button, Modal } from "../../../renderer/shared-ui.js";

export default function LiveChatPanel({ sessionId, overlay = false, condensed = false }) {
  ...
  const timeline = liveChatTimeline(messages, data?.audience?.recent_joins || []);
  useEffect(() => {
    setBody(''); setReply(null); setRulesEdit(null); setShowAudience(false); setShowRules(false); setFailure(''); setSelected(null); setRestrictions(null); setNotice(''); setBusy(false);
  ...
  return <section aria-label="Chat da live" className={`flex h-full min-h-0 min-w-0 flex-col overflow-hidden rounded-2xl ${overlay ? 'bg-black/65 text-white backdrop-blur-sm' : 'border border-px-border bg-px-card'} ${condensed ? 'p-2' : 'p-3'}`}>
    <div className="flex shrink-0 items-center justify-between gap-2"><div className="min-w-0"><h2 className={condensed ? 'sr-only' : 'text-sm font-semibold'}>Chat da live</h2>{Number.isInteger(data?.audience?.viewer_count) && (data.audience.can_view_list ? <button type="button" onClick={() => setShowAudience(true)} className="flex min-h-9 items-center gap-1 text-xs text-purple-300 underline" aria-label={`Ver público: ${data.audience.viewer_count} pessoas na live`}><Users size={13} />{data.audience.viewer_count} na live · Ver público</button> : <span className="flex items-center gap-1 text-xs opacity-75"><Users size={13} aria-hidden="true" />{data.audience.viewer_count} na live</span>)}</div><div className="flex gap-1">{condensed && data?.rules && <button type="button" aria-label="Regras da sala" onClick={() => setShowRules(true)} className="flex min-h-11 min-w-11 items-center justify-center rounded-lg hover:bg-white/10"><BookOpen size={16} /></button>}<button type="button" title="Denunciar live" aria-label="Denunciar live" onClick={() => { setSelected({ reportLive: true }); setReason(''); setFailure(''); }} className="flex min-h-11 min-w-11 items-center justify-center rounded-lg hover:bg-white/10"><Flag size={16} /></button>{canModerate && <button type="button" aria-label="Gerenciar restrições do chat" disabled={busy} onClick={loadRestrictions} className="flex min-h-11 min-w-11 items-center justify-center rounded-lg hover:bg-white/10"><Shield size={16} /></button>}</div></div>
    {!condensed && data?.rules && <details className="mb-2 shrink-0 text-xs"><summary className="cursor-pointer py-2 font-medium">Regras da sala</summary><p className="whitespace-pre-wrap break-words p-2 opacity-75">{data.rules}</p></details>}
    ...
    {!condensed && data?.moderation?.can_manage_rules && <button type="button" onClick={() => setRulesEdit(data?.rules || '')} className="mb-1 self-start text-xs text-purple-300">Editar regras</button>}
    <div ref={list} onScroll={() => { const el = list.current; bottom.current = el.scrollHeight - el.scrollTop - el.clientHeight < 35; if (bottom.current) setUnread(false); }} className={`min-h-0 flex-1 space-y-2 overflow-y-auto overscroll-contain ${overlay ? 'max-h-[26dvh]' : 'max-h-[42dvh]'}`} aria-label="Mensagens recentes">
      {loading ? <p className="p-2 text-xs">Carregando chat…</p> : !data?.enabled ? <p className="p-2 text-xs">O chat está em preparação ou foi pausado pela administração.</p> : !timeline.length ? <p className="p-2 text-xs opacity-75">Seja a primeira pessoa a conversar.</p> : timeline.map(message => message.rowType === 'join' ? <p key={message.rowKey} className="flex items-start gap-1 break-words py-1 text-xs opacity-65"><LogIn size={13} className="mt-0.5 shrink-0" aria-hidden="true" /><span className="min-w-0 [overflow-wrap:anywhere]">@{message.user.username} entrou na live</span></p> : <div key={message.rowKey} className="flex items-start gap-1 text-sm"><p className="min-w-0 flex-1 break-words py-1 [overflow-wrap:anywhere]"><span className="mr-2 font-semibold text-purple-300">{message.user.is_broadcaster && <span className="mr-1 inline-flex items-center gap-1 rounded bg-purple-500/20 px-1 text-[10px] font-medium" title="Pessoa transmitindo esta live"><Radio size={11} aria-hidden="true" />Streamer</span>}{message.user.username}</span>{message.reply_to && <span className="mb-1 block border-l-2 border-purple-400 pl-2 text-xs opacity-60">Respondendo a @{message.reply_to.user?.username}: {message.reply_to.body || 'Mensagem removida'}</span>}{message.deleted_at ? <span className="italic opacity-60">Mensagem removida</span> : message.body}</p>{!message.deleted_at && <button type="button" aria-label={`Opções da mensagem de ${message.user.username}`} onClick={() => { setSelected(message); setReason(''); setFailure(''); }} className="flex h-10 w-9 shrink-0 items-center justify-center rounded-lg hover:bg-white/10"><MoreVertical size={15} /></button>}</div>)}
    </div>
    ...
    <form onSubmit={send} className="mt-2 flex shrink-0 items-end gap-2"><label className="min-w-0 flex-1"><span className="sr-only">Escrever no chat da live</span><textarea value={body} onChange={e => setBody(e.target.value)} onKeyDown={e => { if (e.key === 'Enter' && !e.shiftKey && !e.nativeEvent.isComposing) { e.preventDefault(); if (body.trim() && !busy && data?.can_send) e.currentTarget.form?.requestSubmit(); } }} maxLength={500} rows={!condensed && body.length > 70 ? 3 : 1} disabled={busy || !data?.can_send} placeholder={data?.can_send ? 'Escreva uma mensagem…' : 'Chat indisponível para envio'} className="max-h-28 min-h-11 w-full resize-none rounded-xl border border-white/20 bg-black/30 px-3 py-2 text-base text-white placeholder:text-white/65 disabled:opacity-60" /></label><Button type="submit" aria-label="Enviar mensagem" loading={busy} disabled={!data?.can_send || !body.trim()} className="min-h-11 min-w-11 px-2"><Send size={18} /></Button></form>
    ...
    <Modal isOpen={showAudience && !!data?.audience?.can_view_list} onClose={() => setShowAudience(false)} title="Público da live">{showAudience && data?.audience?.can_view_list && <LiveAudiencePanel key={sessionId} sessionId={sessionId} />}</Modal>
    <Modal isOpen={showRules} onClose={() => setShowRules(false)} title="Regras da sala"><p className="whitespace-pre-wrap break-words text-sm">{data?.rules}</p>{data?.moderation?.can_manage_rules && <Button className="mt-4" onClick={() => { setShowRules(false); setRulesEdit(data?.rules || ''); }}>Editar regras</Button>}</Modal>
  </section>;
}
